...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
	  <div style="float:left; width:50%; margin-top:-20px;">
		...
		<div class="label label-warning span11" style="padding:5px;text-align:center;"><h4><?php echo date_format(date_create($model->tgl_pengajuan),"d F Y"); ?></h4></div>
		<?php echo $form->hiddenField($model,'tgl_pengajuan'); ?>
	  </div>
	  <div style="float:left; width:50%; margin-top:-20px;">	
		<h5>No. Form</h5>
		<div class="label label-success span11" style="padding:5px;text-align:center;"><h4><?php echo $model->kode; ?></h4></div>
		<?php echo $form->hiddenField($model,'kode',array('size'=>25,'maxlength'=>25)); ?>
	  </div>
	</div>
	<hr>	
	...
	<h5>Tanggal :</h5>
		<div style="float:left; width:20%; margin-left:0px">
			<?php $minTgl = Konfig::model()->findByPk(2)->min_tgl_pengajuan; ?>
			<?php $minYear = date("Y", strtotime(date("Y-m-d"))); ?> 
			<?php $minMonth = date("m", strtotime(date("Y-m-d"))); ?> 
			<?php $minDay = date("d", strtotime(date("Y-m-d"))); ?> 
			<?php if(date("m")== 12){
					  $maxYear = date("Y", strtotime(date("Y-m-d") ."+1 year")); 
					  $isThisYear = true;
				  }else{ 
					  $maxYear = date("Y", strtotime(date("Y-m-d"))); 
				      $isThisYear = false; } ?>
			<?php $yearRange = $minYear .':' .$maxYear; ?>
			<?php $minDate = date("Y-m-d", strtotime(date("Y-m-d")."-".$minTgl." day")); ?>
